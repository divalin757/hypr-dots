...
notify_user() {
  ...
  notify-send -e  -h int:value:"$val" -h string:x-canonical-private-synchronous:osd -u low -i "$(get_icon)" "Volume" "$label"
}
...
notify_mic_user() {
  ...
  notify-send -e -h int:value:"$val" -h string:x-canonical-private-synchronous:mic_notif -u low -i "$(get_mic_icon)" "Microphone" "$label"
}
