...
")
  ...
  if ! [[ $actions1 =~ ^( $actionspwd| ..|create-file|create-dir|change-dir|empty-trash|restore-trash|list-trash|find-files|actions:|quit)$ ]]; then
    actions=$(echo "delete rename move-files trash open-files quit" | tr ' ' '\n' | fzf --prompt " > ")
  fi
